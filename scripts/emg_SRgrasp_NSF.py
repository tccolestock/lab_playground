...
"""
emg_SRgrasp_NSF.py: Script is designed to read emg data and execute a joint synergy path plan on the
 Shadow Hand (from open, to closed grip). The EMG data is measured using the Myo Band using the script
 myo-rawNode-right.py for measuring EMG signals from the right forearm.

Known Issues: Currently when the Shadow Hand (SH) is sent the commands to change position based on EMG
 mapping to the joint positions, the SH joints start to oscillate wildly. When running the same joint
 motions using a for loop to simulate the EMG input the SH operates smoothly. Further investigation is
 needed to troubleshoot the problem. (as of 12/16/2016)
...
class JointDictionaries(object):
    ...
    # Full dictionaries for use with a fully working system (after troubleshooting oscillation)
    # hand_start = {
    ...
    # Test dictionaries for troubleshooting the oscillation issue, without having to move all joints
    hand_start = {'rh_RFJ3': 0.040169677117662395}
    hand_close = {'rh_RFJ3': 0.6358242015720096}
...
# Test dictionary with simplified values (hand_start below)
hand_start = {
...
if __name__ == '__main__':
    ...
    # Test for loop to prove joints don't oscillate under this condition
    ...
    #     print("dict: \n")
    #     print(jointdict.move_dict)
    #     print("duration: %f" % jointdict.duration)
    #     print("\n")
    #     # time.delay(0.1)
